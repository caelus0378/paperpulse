...
def _normalize_subfield(sf: str) -> str:
    """规范化子领域名称——LLM 有时会省略括号等，统一匹配到标准名"""
    if not sf:
        return "深度学习架构与训练方法"
    sf = sf.strip()
    # 移除 LLM 可能添加的前缀描述
    prefixes = ["包含", "主要", "属于", "属于应用", "应用于", "应用领域", "应用", "主要是"]
    for pre in prefixes:
        if sf.startswith(pre):
            sf = sf[len(pre):].strip()
    # 标准化映射
    norm_map = {
        "大语言模型与Agent": "大语言模型与Agent (LLM/Agent)",
        "大语言模型": "大语言模型与Agent (LLM/Agent)",
        "LLM": "大语言模型与Agent (LLM/Agent)",
        "计算机视觉与图像": "计算机视觉与图像 (CV)",
        "计算机视觉": "计算机视觉与图像 (CV)",
        "CV": "计算机视觉与图像 (CV)",
        "多模态学习": "多模态学习 (Multimodal)",
        "多模态": "多模态学习 (Multimodal)",
        "机器人学与控制": "机器人学与控制 (Robotics)",
        "机器人": "机器人学与控制 (Robotics)",
        "代码与软件工程": "代码与软件工程 (Code/SE)",
        "软件工程": "代码与软件工程 (Code/SE)",
        "生成模型": "生成模型 (GAN/Diffusion/ViT)",
        "生成模型 (GAN/Diffusion)": "生成模型 (GAN/Diffusion/ViT)",
        "教育技术": "AI应用-教育技术",
        "信号处理与通信": "AI应用-信号处理与通信",
        "金融经济学": "AI应用-金融经济学",
        "金融科技": "AI应用-金融经济学",
        "网络安全": "AI应用-网络安全与密码学",
        "密码学": "AI应用-网络安全与密码学",
...
# ============================================================
SUBFIELDS = [
    "大语言模型与Agent (LLM/Agent)",
    "自然语言处理 (NLP)",
    "计算机视觉与图像 (CV)",
    "强化学习 (RL)",
    "生成模型 (GAN/Diffusion/ViT)",
    "多模态学习 (Multimodal)",
    "AI安全与可解释性",
    "机器学习理论与优化",
    "机器人学与控制 (Robotics)",
...
4. **严禁使用"其他"、"Others"等模糊分类**——必须以具体方法/技术归类
5. 尽量根据论文使用的**核心方法/技术**选择子领域，而非仅凭应用场景"""
...
def classify_papers(papers: list[dict], batch_size: int = 15) -> list[dict]:
    ...
    for i in range(0, len(papers), batch_size):
        ...
        try:
            ...
            # 规范化子领域名称
            for r in batch_results:
                r["subfield"] = _normalize_subfield(r.get("subfield", ""))
            results.extend(batch_results)
            print(f"    [OK] 完成 {len(batch_results)} 篇分类")
...
def _parse_classification_response(response: str, batch: list[dict]) -> list[dict]:
    ...
    for p in batch:
        ...
        if aid in result_map:
            results.append({
                "arxiv_id": aid,
                "subfield": result_map[aid].get("subfield", "深度学习架构与训练方法"),
